...
public interface UserModuleProgressRepository extends JpaRepository<UserModuleProgress, Integer> {
    ...
    // Count completed modules within a date range
    @Query("SELECT COUNT(p) FROM UserModuleProgress p WHERE p.isCompleted = true AND p.completedAt >= :startDate AND p.completedAt < :endDate")
    long countCompletedModulesBetweenDates(@Param("startDate") LocalDateTime startDate, @Param("endDate") LocalDateTime endDate);
}
