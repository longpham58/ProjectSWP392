...
const CreateNotificationModal: React.FC<CreateNotificationModalProps> = ({ onClose }) => {
  const { createNotification, loading } = useTrainerNotificationStore();
      ...
      <div className="bg-white rounded-lg shadow-xl p-8 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        {recipient === 'STUDENTS' && (
          <div className="mb-6">
            ...
            {loadingCourses ? (
              <div className="text-center py-4 text-gray-500">Đang tải danh sách lớp học...</div>
            ) : availableClasses.length === 0 ? (
              <div className="text-center py-4 text-gray-500">Không có lớp học nào</div>
            ) : (
              <>
                  ...
                  {availableClasses.map((course) => (
                    <div
                      ...
                      className={`p-3 rounded-lg border-2 transition ${
                        selectedClasses.includes(course.code)
                    ...
                    >
                      <label className="flex items-center cursor-pointer">
                        <input
                          type="checkbox"
                          checked={selectedClasses.includes(course.code)}
                          onChange={() => toggleClass(course.code)}
                          className="w-5 h-5 text-blue-600 rounded focus:ring-blue-500 mr-3"
                        />
                        <div className="flex-1">
